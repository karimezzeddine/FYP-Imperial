function [SignalX,SignalY,SignalZ,SignalTime] = segment_accel_volley_trial(Filename,colaccX,colaccY,colaccZ)
...
SignalX=zeros(120,length(bpks)); % array to store each shot in one column, so 33 columns for 33 shots and 1081 rows for 1081 data points which represent 5s = 1 shot
